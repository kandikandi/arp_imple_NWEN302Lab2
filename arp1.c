...
void insert_new_map(struct arphdr *arpptr){
    ...
    if(index==-1){
        ...
        addresses[index]->ttl = time(NULL);
    }


}

/*Called everytime a packet is recieved, clears out old addresses that haven't been used in EXPIRY time*/
void delete_used_maps(int removal_type){
    ...
    if(removal_type==1){
        ...
        for(i = 0; i < num_maps; i++){              
            if(time_now-addresses[i]->ttl>=EXPIRATION){
                addresses[i] = NULL;
...
void print_one_map(struct map* m){
    ...
    fprintf(stderr,"The MAC for Node %d is %s\n",m->ip_addr,mac);
    fprintf(stderr,"Its time to live started at %ld seconds\n\n",m->ttl);
}
